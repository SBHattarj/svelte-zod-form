...
| date   | `function (date: string | Date): string` | function to format a `Date` or a date string to proper format to be used by the input date element. This is needed to properly use date inputs as the values object is not aware if you are using date input or datetime-locale input, the values can't provide you with a formatted string by default |
| datetime | `function (date: string | Date): string` | this function is similar to date function but should be used to format a `Date` or a date string to proper format to be used by the datetime-locale input element. |
| dateformat | `function (date: string | Date): string` | A helper function to format a date to any other format. use `$H` for hour, `$M` for month, `$D` for date, `$H` for hour and `$M` for minute. It doesn't have any other support as not required for date and datetime inputs. This is just for completeness sake, maybe removed in the future. |
